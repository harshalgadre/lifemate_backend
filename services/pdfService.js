    ...
    try {
      ...
      if (personalInfo.address?.city && personalInfo.address?.state) {
        doc.fontSize(10)
           .font('Helvetica')
           .fillColor(primaryColor)
           .text(`${personalInfo.address.city}, ${personalInfo.address.state}, ${personalInfo.address.country || 'India'}`, { align: 'center' });
      }
      ...
      if (personalInfo.linkedIn) contactInfo.push(personalInfo.linkedIn.replace('https://linkedin.com/in/', 'linkedin.com/'));
      if (personalInfo.github) contactInfo.push(personalInfo.github.replace('https://github.com/', 'github.com/'));
      ...
      if (resumeData.education && resumeData.education.length > 0) {
        const visibleEdu = resumeData.education.filter(edu => edu.isVisible !== false);
        if (visibleEdu.length > 0) {
          ...
          visibleEdu.forEach((edu, index) => {
            ...
            if (edu.yearOfCompletion) {
              ...
              doc.text(`    ${startYear}-${edu.yearOfCompletion}`, { align: 'left' });
            }
      ...
      if (resumeData.skills && resumeData.skills.length > 0) {
        const visibleSkills = resumeData.skills.filter(skill => skill.isVisible !== false);
      ...
      if (resumeData.workExperience && resumeData.workExperience.length > 0) {
        const visibleExp = resumeData.workExperience.filter(exp => exp.isVisible !== false);
        if (visibleExp.length > 0) {
          ...
          visibleExp.forEach((exp, index) => {
            ...
            doc.text(' '.repeat(Math.max(1, Math.floor(spacesNeeded / 3))) + dateText);
      ...
      if (resumeData.projects && resumeData.projects.length > 0) {
        const visibleProjects = resumeData.projects.filter(proj => proj.isVisible !== false);
          ...
          visibleProjects.forEach((proj, index) => {
            ...
            if (proj.technologies && proj.technologies.length > 0) {
              doc.font('Helvetica-Oblique')
                 .fontSize(9)
                 .fillColor('#333333')
                 .text(` | ${proj.technologies.join(', ')}`, { continued: true });
            }
      ...
      if (resumeData.customSections && resumeData.customSections.length > 0) {
        const visibleSections = resumeData.customSections.filter(sec => sec.isVisible !== false);
      ...
      if (resumeData.certifications && resumeData.certifications.length > 0) {
        const visibleCerts = resumeData.certifications.filter(cert => cert.isVisible !== false);
        if (visibleCerts.length > 0) {
          ...
          visibleCerts.forEach((cert, index) => {
            doc.fontSize(9)
               .font('Helvetica')
               .fillColor(primaryColor)
               .text(`• ${cert.name} - ${cert.issuingOrganization}`, { indent: 0 });
      ...
      if (resumeData.languages && resumeData.languages.length > 0) {
        const visibleLangs = resumeData.languages.filter(lang => lang.isVisible !== false);
        if (visibleLangs.length > 0) {
          addSectionHeader('Languages');
          
          const langText = visibleLangs.map(lang => `${lang.name} (${lang.proficiency})`).join(', ');
          doc.fontSize(9)
             .font('Helvetica')
             .fillColor(primaryColor)
             .text(langText);
