...
exports.listMyApplications = async (req, res) => {
  try {
    ...
    return successResponse(res, 200, 'Applications fetched', { items, page, limit, total, pages: Math.ceil(total / limit) });
  } catch (err) {
...
exports.listEmployerApplications = async (req, res) => {
  try {
    ...
    const [items, total] = await Promise.all([
      Application.find(filters).populate('job jobSeeker').sort(sort).skip(skip).limit(limit),
      Application.countDocuments(filters),
    ]);

    return successResponse(res, 200, 'Employer applications fetched', { items, page, limit, total, pages: Math.ceil(total / limit) });
  } catch (err) {
...
exports.getById = async (req, res) => {
  try {
    const application = await Application.findById(req.params.id).populate('job jobSeeker employer');
    ...
    if (!isAdmin) {
      const jobSeeker = await JobSeeker.findOne({ user: req.user._id });
      const employer = await Employer.findOne({ user: req.user._id });
      const ownsAsSeeker = jobSeeker && application.jobSeeker.toString() === jobSeeker._id.toString();
      const ownsAsEmployer = employer && application.employer.toString() === employer._id.toString();
      if (!ownsAsSeeker && !ownsAsEmployer) {
        return forbiddenResponse(res, 'Not authorized to view this application');
      }
    }
  ...
  try {
    ...
    if (req.user.role !== 'admin') {
      ...
      if (!employer || application.employer.toString() !== employer._id.toString()) {
        return forbiddenResponse(res, 'Not authorized to update this application');
      }
    }
    ...
    application.history.push({ status, note, by: req.user._id, at: new Date() });
